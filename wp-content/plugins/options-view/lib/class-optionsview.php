...
class OptionsView {
	...
	public function settings_link( $links, $file ) {
		...
		if ( $file === $this_plugin ) {
			$links[] = '<a href="' . admin_url( 'tools.php?page=optionsview' ) . '">' . __( 'View', 'options-view' ) . '</a>';
		}
			return $links;
	}
	...
	public function plugin_options() {
		...
		if ( ! current_user_can( 'manage_options' ) ) {
			wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'options-view' ) );
		}
		...
		<h2>Options View</h2>
			...
			<div style="margin: 5px; padding: 5px;">
				...
				submit_button( __( 'Update', 'options-view' ), 'primary', 'options-view-update1', false, array( 'form' => 'optionsview_forms' ) );
				?>
				&nbsp;&nbsp;&nbsp;
				<?php
				submit_button( __( 'Delete', 'options-view' ), 'large', 'options-view-delete1', false, array( 'form' => 'optionsview_forms' ) );
				do_action( 'opv_per_page_set' );
				$options_view_list_table->display();
				submit_button( __( 'Update', 'options-view' ), 'primary', 'options-view-update2', false, array( 'form' => 'optionsview_forms' ) );
				?>
				&nbsp;&nbsp;&nbsp;
				<?php
				submit_button( __( 'Delete', 'options-view' ), 'large', 'options-view-delete2', false, array( 'form' => 'optionsview_forms' ) );
				?>
			</div>
	...
	public function option_select_form() {
		...
		<?php submit_button( __( 'Select', 'options-view' ), 'large', 'options-view-select', false ); ?>
		</form>
		<?php
	}
	...
	public function filter_form() {
		...
		if ( ! $search_text ) {
			?>
			<input style="vertical-align: middle;" name="search_text" type="text" value="" placeholder="<?php echo esc_attr__( 'Search', 'options-view' ); ?>">
			<?php
		} else {
			?>
		...
		submit_button( __( 'Filter', 'options-view' ), 'large', 'options-view-filter', false );
		?>
		</form>
		</div>
		<?php
	...
	public function per_page_set() {
		...
		?>
		<div style="margin: 0px; text-align: right;">
			<?php esc_html_e( 'Number of items per page:', 'options-view' ); ?><input type="number" step="1" min="1" max="9999" style="width: 80px;" name="per_page" value="<?php echo esc_attr( get_option( 'opv_per_page', 20 ) ); ?>" form="optionsview_forms" />
			<?php submit_button( __( 'Change', 'options-view' ), 'large', 'per_page_change', false, array( 'form' => 'optionsview_forms' ) ); ?>
		</div>
		<?php
	}
	...
	private function credit() {
		...
		$plugin_version = __( 'Version:', 'options-view' ) . ' ' . $plugin_ver_num;
		...
		echo esc_html( sprintf( __( 'Translations for %s', 'options-view' ), $plugin_name ) );
		?>
		...
		<button type="button" style="margin: 5px; padding: 5px;" onclick="window.open('<?php echo esc_url( $donate ); ?>')"><?php esc_html_e( 'Donate to this plugin &#187;', 'options-view' ); ?></button>
		</div>
