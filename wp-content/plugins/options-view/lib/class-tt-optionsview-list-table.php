 ...
 * @subpackage OptionsView List Table
 ...
 * List table
 */
class TT_OptionsView_List_Table extends WP_List_Table {
	...
	private function read_data() {
		...
		if ( isset( $_POST['options-view-filter'] ) && ! empty( $_POST['options-view-filter'] ) ) {
			if ( check_admin_referer( 'opv_filter', 'options_view_filter' ) ) {
				if ( ! empty( $_POST['user_id'] ) ) {
					update_option( 'opv_filter_user', absint( $_POST['user_id'] ) );
				} else {
					update_option( 'opv_filter_user', null );
				}
				if ( ! empty( $_POST['search_text'] ) ) {
					$search_text = sanitize_text_field( wp_unslash( $_POST['search_text'] ) );
					update_option( 'optionsview_search_text', $search_text );
				} else {
		...
		if ( 'options' == get_option( 'options-view', 'options' ) ) {
			...
			if ( ! empty( $options ) ) {
				foreach ( $options as $option ) {
					$search = false;
					if ( $search_text ) {
						if ( false !== strpos( $option->option_name, $search_text ) ) {
							$search = true;
						}
						if ( false !== strpos( $option->option_id, $search_text ) ) {
							$search = true;
						}
					} else {
						$search = true;
			...
			if ( ! empty( $user_filter ) ) {
				...
				$user_options = $wpdb->get_results( "SELECT * FROM {$wpdb->prefix}usermeta ORDER BY meta_key" );
			}
			if ( ! empty( $user_options ) ) {
				foreach ( $user_options as $option ) {
					$search = false;
					if ( ! is_null( $search_text ) ) {
						if ( false !== strpos( $option->meta_key, $search_text ) ) {
							$search = true;
						}
						if ( false !== strpos( $option->umeta_id, $search_text ) ) {
							$search = true;
						}
						if ( false !== strpos( $option->user_id, $search_text ) ) {
							$search = true;
						}
					} else {
						$search = true;
					...
					if ( $search ) {
						$listtable_array[] = array(
							'ID'    => $option->umeta_id,
							'title' => $option->meta_key,
							'ids'   => $option->umeta_id,
							'user'  => $option->user_id,
							'edit'  => $option->meta_value,
						);
					}
				}
			}
	...
	public function column_default( $item, $column_name ) {
		...
		switch ( $column_name ) {
			...
			case 'edit':
				...
				$structure_none = false;
				$structure = '<details>';
				$structure .= '<summary><strong>' . __( 'Structure', 'options-view' ) . '</strong></summary>';
				$structure .= '<pre style="width: 100%; white-space: pre-wrap;">';
				if ( is_array( $value ) || is_object( $value ) ) {
					$structure .= print_r( $value, true );
				} elseif ( is_string( $value ) &&
						is_array( json_decode( $value, true ) ) &&
						json_last_error() == JSON_ERROR_NONE ) {

						$value2 = json_decode( $value, true );
						$structure .= print_r( $value2, true );
				} else {
					$structure_none = true;
				}
				$structure .= '</pre>';
				...
				return $structure . sprintf(
					'<textarea name="%1$s[%2$s]" %4$s form="optionsview_forms">%3$s</textarea>',
					/*%1$s*/ $this->_args['singular'],
					/*%2$s*/ $item['ID'],
					/*%3$s*/ $item[ $column_name ],
					/*%4$s*/ 'style="width: 100%;"'
				);
			default:
	...
	public function column_cb( $item ) {
		...
		return sprintf(
			'<input type="checkbox" name="bulk_options_check[]" value="%1$s" form="optionsview_forms" />',
			/*%1$s*/ $item['ID']
		);
	}
	...
	public function get_columns() {
		...
		if ( 'options' == get_option( 'options-view', 'options' ) ) {
			$columns = array(
				...
				'title' => __( 'Title', 'options-view' ),
				'ids'   => 'ID',
				'edit'  => __( 'Edit', 'options-view' ),
			);
		} else if ( 'user_options' == get_option( 'options-view', 'options' ) ) {
			$columns = array(
				'cb'    => '<input type="checkbox" />', /* Render a checkbox instead of text */
	...
	public function print_column_headers( $with_id = true ) {
		...
		if ( isset( $_SERVER['HTTP_HOST'] ) && ! empty( $_SERVER['HTTP_HOST'] ) &&
			...
			/* Customize for nonce */
			$current_url = wp_nonce_url( $current_url, 'opv_sort_nonce' );
		} else {
			wp_die();
		}
		...
		if ( wp_verify_nonce( $nonce, 'opv_sort_nonce' ) ) {
		...
		foreach ( $columns as $column_key => $column_display_name ) {
			...
			if ( isset( $sortable[ $column_key ] ) ) {
				...
				if ( $current_orderby === $orderby ) {
					...
					/* translators: Hidden accessibility text. */
					$asc_text = __( 'Sort ascending.', 'options-view' );
					/* translators: Hidden accessibility text. */
					$desc_text  = __( 'Sort descending.', 'options-view' );
	...
	public function prepare_items() {
		 ...
		 */
		$per_page = get_option( 'opv_per_page', 20 );
