		...
		if ( '' != $valores['image_id'] && 0 != $valores['image_id'] ) {
			...
			if ('' != $valores['image_id'] && 0 != $valores['image_id'] ) {
				...
				if ( 'image/svg+xml' == $default['mime-type'] ) {
					...
					if ( '' == $valores['alt'] ) {
						// Alt vacío: usar texto por defecto y registrar problema de accesibilidad
						$valores['alt'] = 'Imagen sin título';
						$area_hidden = ' aria-hidden="true"';
						$valores['area-hidden'] = 'true';
						
						// Log de accesibilidad para tracking
						if (function_exists('ikg_accessibility')) {
							ikg_accessibility('Imagen sin texto alternativo (alt)', [
								'image_id' => $valores['image_id'],
								'url' => $default['url'] ?? '',
								'file' => $default['file'] ?? '',
								'page_id' => get_the_ID(),
...
/**
 * Helper function para mostrar imagen
 * 
 * @param mixed $datos ID de imagen, array de configuración, o nombre de campo ACF
 */
