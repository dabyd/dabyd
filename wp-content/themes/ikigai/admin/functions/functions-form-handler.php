...
function ikg_handle_form_submission() {
    ...
    // 3. Obtener email destinatario del formulario (campo ACF)
    $email_destinatario = '';
    
    // Intentar obtener con ACF primero
    if (function_exists('get_field')) {
        $email_destinatario = get_field('email_destinatario', $form_id);
    }
    
    // Fallback a post_meta si ACF no está disponible
    if (empty($email_destinatario)) {
        $email_destinatario = get_post_meta($form_id, 'email_destinatario', true);
    }
    
    // Si sigue vacío, usar email del admin
    if (empty($email_destinatario)) {
